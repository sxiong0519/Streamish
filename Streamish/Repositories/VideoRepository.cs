    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
        ...
        public List<Video> GetAll()
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                          SELECT v.Id AS VideoId, Title, Description, Url, v.DateCreated AS VideoDateCreated, UserProfileId, Name, Email, ImageUrl, 
        ...
        public Video GetVideoWithComments(int id)
        {
            using (var conn = Connection)
            {
                conn.Open();
                ...
                SELECT v.Id AS VideoId, v.Title, v.Description, v.Url, 
                       ...
                       upc.Name AS CommentUser
                  FROM Video v 
                       JOIN UserProfile up ON v.UserProfileId = up.Id
                       LEFT JOIN Comment c on c.VideoId = v.id
                       LEFT JOIN UserProfile upc ON upc.Id = c.UserProfileId
                       WHERE v.Id = @id
            ";
                    ...
                    while (reader.Read())
                    {
                        ...
                        if (DbUtils.IsNotDbNull(reader, "CommentId"))
                        {
                            video.Comments.Add(new Comment
                            {
                                ...
                                UserProfile = new UserProfile
                                {
                                    Name = DbUtils.GetString(reader, "CommentUser")
                                }
                            });
                        };
                    }
        ...
        public List<Video> HotSearch(DateTime since)
        {
            using (var conn = Connection)
            {
                conn.Open();
              ...
              SELECT v.Id, v.Title, v.Description, v.Url, v.DateCreated AS VideoDateCreated, v.UserProfileId,
               ...
               WHERE v.DateCreated >= @Criterion";
                    ...
                    cmd.CommandText = sql;
                    DbUtils.AddParameter(cmd, "@Criterion", since);
                    var reader = cmd.ExecuteReader();
